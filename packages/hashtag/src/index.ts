...
export const Hashtag = (options?: Partial<Mark>) =>
	Mark({
		...
		toDOM(mark: PMMark) {
			return ['a', { class: 'tag', href: mark.attrs.href }, 0];
		},
			...
			return [
				new Plugin({
					...
					appendTransaction(transactions, oldState, newState) {
						...
						let textChange = transactions.some(({ steps }) =>
							steps.some(step => step instanceof ReplaceStep || step instanceof ReplaceAroundStep)
						);
						...
						const sameParent =
							newState.selection.from - newState.selection.$from.parentOffset ==
							oldState.selection.from - oldState.selection.$from.parentOffset;
						...
						function hashify($from) {
							...
							const endOfTextRange = Math.min(
								$from.pos + 128,
								$from.pos - $from.parentOffset + $from.parent.nodeSize - 2
							);
