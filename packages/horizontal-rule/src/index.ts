import { canInsert, Node } from '@ibra-kdbra/editor';
...
export const HorizontalRule = (options?: Partial<Node>) =>
	Node({
		name: 'horizontalrule',
		...
		parseDOM: [
			{
				tag: 'hr',
				getAttrs: (dom: HTMLHRElement) => ({ type: dom.dataset.type }),
			},
		],
		toDOM(node: PMNode) {
			return ['hr', { 'data-type': node.attrs.type }];
		},
