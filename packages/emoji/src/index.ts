...
const SHORT_EMOJI_REGEX = /(\p{EPres}|\p{ExtPict})/gu;
const EMOJI_REGEX = emojiRegex();

const emoji_shortcode_inputRegex = /:([a-zA-Z0-9_+-]+):$/;
const emoji_shortcode_pasteRegex = /:([a-zA-Z0-9_+-]+):/g;
...
export const Emoji = (options?: Partial<Node>) =>
	Node({
		...
		inputRules({ nodeType }) {
			return [
				new InputRule(EMOJI_REGEX, function insertEmoji(
					state: EditorState,
